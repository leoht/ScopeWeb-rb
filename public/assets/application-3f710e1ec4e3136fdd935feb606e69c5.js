...
(function() {
  var SOCKER_SERVER_ADDR, SOCKER_SERVER_PORT;
  ...
  window.Api = (function() {
    ...
    Api.prototype.beginConnection = function(callback) {
      this.socket = new WebSocket('ws://' + SOCKER_SERVER_ADDR + ':' + SOCKER_SERVER_PORT);
      this.socket.onmessage = (function(_this) {
        return function(message) {
          ...
          if (data.name === 'api.associated.token') {
            return _this.token = data.data.token;
          } else {
            if (data.name in _this.callbacks) {
              cb = _this.callbacks[data.name];
              return cb(data.data);
            }
          }
        };
    ...
    Api.prototype.on = function(message_name, callback) {
      return this.callbacks[message_name] = callback;
    };
...
(function() {
  ...
  api.on(ASSOCIATION_INITIATED_WITH_CODE, function(data) {
    return console.log('Received code :' + data.code);
  });
  ...
  api.on(SET_VOLUME, function(data) {
    return mediaPlayer.volume = data.volume;
  });

}).call(this);
(function() {
  var call_ios_function, open_custom_url_in_iframe;

  open_custom_url_in_iframe = function(src) {
    var new_frame_element, root_element;
    ...
    return new_frame_element.parentNode.removeChild(new_frame_element);
  };

  call_ios_function = function(function_name, args, success, error) {
    var call_info, url;
    url = 'js2ios://';
    call_info = {};
    ...
    return open_custom_url_in_iframe(url);
  };
...
(function() {
  window.initializeMediaPlayer = function() {
    window.mediaPlayer = document.getElementById('player');
    return window.mediaPlayer.controls = false;
  };

  window.togglePlayPause = function() {
    var btn;
    btn = document.getElementById('play-pause-button');
    if (window.mediaPlayer.paused || window.mediaPlayer.ended) {
      btn.title = 'Pause';
      btn.innerHTML = 'Pause';
      btn.className = 'pause';
      window.mediaPlayer.play();
      return window.playingTimer = setInterval(function() {
        return api.send('api.playing.current_timecode', {
      ...
      return clearInterval(window.playingTimer);
    }
  };
